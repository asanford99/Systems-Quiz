...
void changeMax(int maxNumPar, int gamesPlayedPar, int* gamesDataPar) {
    ...
    //reads input from user, validates the input, and overwrites the data in the maxNum file
    scanf("%s", userInput);
    ...
    sprintf(userInput, "%d", userNum);

    maxNumFile = fopen("maxNum.txt", "wt+");


    fwrite(userInput, 1, sizeof(userInput), maxNumFile);
...
int main() {
    ...
    //the main function will read the maxNum file, saved from the previous play, and input that into the maxNum variable
    int maxNum;
    int* gamesData;
    char fileInput[2] = "";
    FILE *maxNumFile;

    maxNumFile = fopen("maxNum.txt", "r");

    fscanf(maxNumFile, "%s", fileInput);

    fclose(maxNumFile);

    if (!strtod(fileInput, NULL)){
